...
content("נתוני המחקר", [
    ...
    "משתנים: רציפים — משקל-גוף, משקלי-ניסיון, Total, Dots; קטגוריים — מין, ציוד, פדרציה, קטגוריית-משקל, Tested.",
    "אתגרים וטיפול: אותו מתחרה מופיע בכמה תחרויות (תלות בין שורות) → המבחנים הפורמליים ירוצו עם dedup לפי-מתחרה; התוצאות הראשוניות כאן ברמת-ניסיון/שורה. נועלים snapshot של הנתונים לשחזור.",
])
...
content("שאלת החקר", [
    ...
    "\tH4 — ניבוי (לעבודה הסופית): כמה מהכוח ניתן לנבא ממשתני-השליטה (משקל, ציוד, מין, גיל)?",
], lead="ארבע השערות, סיפור אחד — H1/H2 \"שני המספרים\", ו-H3/H4 גבולות-הכוח והניבוי")
...
content("שיטות החקר", [
    ...
    'תיקון להשוואות מרובות: Bonferroni/Holm/Šidák (FWER) + Benjamini-Hochberg (FDR). בגלל n≈3.9M מובילים בגודל-אפקט+CI, לא ב-p.',
], lead_size=20)
...
fig_slide("תוצאות ראשוניות (2): חיתוך-משקל",
          'מתחת לסף 83 ק"ג (מחלקות IPF+USAPL, גברים) הצטברות חדה: יחס-לוג לאחר ניקוי-עיגול (כבר הוחל) +1.92 ± 0.04 — פי ~6.8 ממש-מתחת לעומת ממש-מעל. בבקרה שאינה סף (91 ק"ג): −0.21 ± 0.03. עקבי עם חיתוך-משקל; אישוש פורמלי (McCrary) בהמשך.',
          "fig_bunching.png", 9.8/4.3, 11.2)

# ---- Slide 8: results H3 (allometry) ----
fig_slide("תוצאות ראשוניות (3): קנה-מידה אלומטרי",
          'קנה-המידה שונה בין המינים: b≈0.72 (R²=0.36) גברים / 0.49 (R²=0.19) נשים, מול 2/3 האיזומטרי (רווח-סמך ל-b צר מאוד, ±<0.01 — n עצום). ייבדק פורמלית במבחן Wald (SE חסין); b<0.5 לנשים — אומדן ראשוני, ייתכן אפקט טווח-משקל.',
