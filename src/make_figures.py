#!/usr/bin/env python3
"""
מייצר גרפים אמיתיים מ-OpenPowerlifting למצגת האמצע (תוצאות ראשוניות).
תוויות צירים באנגלית בכוונה (להימנע מבעיות bidi של matplotlib);
ההסבר העברי מופיע כטקסט בשקף עצמו.
קלט: /tmp/opl-deck/opl.csv  |  פלט: PNG-ים בתיקיית המצגת + הדפסת מספרים.

עדכון אחרי ביקורת (20 סוכנים + codex + gemini):
- H2: חלון-גבול נכון (L-0.5, L]; יחס-לוג מנוקה-עיגול (de-heaped); פלצבו = נקודת-בקרה אמיתית שאינה סף (91), במקום 100 (גבול ישן). כותרת ניטרלית + מקרא.
- H5: סינון Event=='SBD' (טוטאל מלא בלבד) + קו-ייחוס איזומטרי b=2/3; נשים בכתום (לא אדום).
- גדלים/אספקט מותאמים כך שלא ייחתכו בשקף.
"""
...
print(f"  {len(df):,} שורות")

# ---------- H1: קוונטיזציה לרשת 2.5 ק"ג ----------
...
print(f"\n[H1] ניסיונות תקפים: {len(att):,} | על רשת 2.5: {pct_grid:.2f}%")
...
print("  שמר fig_quantization.png")

# ---------- H2: bunching מתחת לסף-מחלקה + נקודת-בקרה ----------
# סינון לסכמת IPF בלבד (IPF + USAPL): אותן מחלקות 83/93/105; 91 אינו סף.
# USPA הוצא — הוא משתמש במחלקות עגולות שונות (82.5/90/100) שמזהמות את הניתוח.
ipf = df[(df["ParentFederation"]=="IPF") | (df["Federation"].isin(["IPF","USAPL"]))]
men = ipf[ipf["Sex"]=="M"]["BodyweightKg"].dropna().to_numpy()
print(f"\n[H2] גברים IPF+USAPL: {len(men):,}")

def asym(bw, L, w=0.5, deheap=False):
    """חלון-גבול נכון: below=(L-w, L], above=(L, L+w]. de-heap=הסרת כפולות 0.5 (מספרים עגולים)."""
    if deheap:
...
print(f"  שמר fig_allometry.png | SBD b: men={slopes['M']:.3f} women={slopes['F']:.3f} (n={len(al):,})")
