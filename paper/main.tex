...
answered with a course-appropriate test and led by effect size (at $n\!\approx\!3.9$M
...
deviance, agrees ($5.0\times10^7$, $\mathrm{df}=4$). The 3.8\% off-grid loads are largely a units artifact: of the
...
(all rows; counts over the 0.5~kg windows just below/above each limit). Last row
is a non-limit control.}
\label{tab:limits}
\begin{tabular}{lccc}
\toprule
...
start a sixth of the bootstrap fits diverge and inflate the upper limit. \textbf{Time
...
standard errors. At $n=300$ all three normality tests the course names agree in rejecting
...
$\chi^2$; and no course tool addresses a bounded tail, since the sample maximum is
