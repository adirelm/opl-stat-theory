...
total ($R^2=0.70$, random forest, leakage-safe grouped CV). These choices leave measurable traces in the data. Code and data:
...
numbers: the load chosen for each attempt, and the bodyweight at weigh-in. Both
interact with a sharp administrative rule. We ask whether these
...
\textbf{Contribution.} The project follows a single thread, ``the two numbers and the limits of
strength,'' and uses both halves of the course, classical inference and machine
learning. Our contributions are: (1)~a rounded-likelihood
...
standard multiple-testing corrections (Bonferroni, Holm, and \v{S}id\'ak for familywise error, and
Benjamini--Hochberg for the false-discovery rate) to every pre-declared family of
...
unbounded $120{+}$ class where the incentive to cut is weakest.
...
$\mathrm{AUC}=0.58$ (PR-AUC $0.88$, balanced accuracy $0.54$). The signal is weak,
...
inflates significance. \textbf{Residual normality.} The H3 residuals are left-skewed
...
normal by the central limit theorem, and we report HC3-robust standard errors. A
formal normality test rejects beyond the smallest samples, as expected at this
scale, so we treat it as a diagnostic and lead with effect sizes.
