...
- **H6 — predicting strength (covers the course's ML / learning half):** how well is `TotalKg`/`Dots` predicted from the controllable + basic variables (bodyweight, sex, equipment, age)? **Multiple linear regression** (log terms + interactions) vs a **Random Forest** baseline, evaluated on **held-out lifters** with **R² / Adjusted R² / RMSE / k-fold CV** + feature importance. On-story: "how predictable is the strength a lifter produces, and which inputs matter." (Lectures 3 + 10.)
...
**Stage 9 — prediction / learning (H6; the course's ML half).** Target `TotalKg` (and/or `Dots`). Features: bodyweight, sex, equipment, age (+ federation/era). Models: **multiple linear regression** (log transforms, Sex×log(BW) interaction, one-hot for categoricals) and a **Random Forest** baseline. Evaluate with a **train/test split grouped by lifter** (no leakage) + **k-fold CV**; report **R² / Adjusted R² / RMSE / MAE**, compare linear-vs-RF, and **feature / permutation importance**. Check **multicollinearity (VIF / correlation matrix)** on the linear model (Lecture 10). Keep it modest and on-story (predicting the strength behind "the two numbers") — one focused model, not a model zoo.
...
- **Prediction / learning (now INCLUDED — Stage 9 / H6):** a modest regression + Random Forest strength-prediction with R²/Adjusted-R²/CV covers the course's ML half (Lectures 3 + 10) and the learning-evaluation metrics. A full classification model zoo (trees/XGBoost/NN) stays out — one focused predictive model that serves the story is enough.
